...
public class ElasticsearchTests {
    ...
    public void testInsert() {
        discussRepository.save(discussPostMapper.selectDiscussPostById(241)); // 插入id为241的一条帖子数据
    ...
    @Test
    public void testUpdate() {  //修改其实就是覆盖。。。
    ...
    @Test
    public void testDelete() { // 我还是注掉吧，删除有点害怕哈哈
        ...
        Page<DiscussPost> page = elasticTemplate.queryForPage(searchQuery, DiscussPost.class, new SearchResultMapper() {
            ...
            public <T> AggregatedPage<T> mapResults(SearchResponse searchResponse, Class<T> aClass, Pageable pageable) {
                ...
                for (SearchHit hit : hits) {
                    ...
                    if (titleField != null) {
                        post.setTitle(titleField.getFragments()[0].toString()); // 把原title覆盖
                    }

                    HighlightField contentField = hit.getHighlightFields().get("content");
                    if (contentField != null) {
                        post.setContent(contentField.getFragments()[0].toString()); // 把原content覆盖
                    }
